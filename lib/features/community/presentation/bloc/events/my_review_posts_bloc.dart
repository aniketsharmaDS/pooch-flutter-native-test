...
class MyReviewPostsBlocActions {
  static const likeEvent = 'LiKE_EVENT';
  static const unLikeEvent = 'UNLIKE_EVENT';
}

class MyReviewPostsBloc extends PaginationBloc<MySubmittedPostsModel> {
  final CommunityRepository communityRepository;
  late final GlobalUpdateBus<dynamic> _bus;
  late final StreamSubscription _busSub;
  static const String _source = 'MyReviewPostsBloc';
  MyReviewPostsBloc({required this.communityRepository})
    : super(
        fetchPage:
            ({
              required int page,
              ...
              log(
                'MyReviewPostsBloc filters in MyReviewPostsBloc fetchPage: $filters',
              );
              final MyPostsApiResponse response = await communityRepository
                  .myAllPosts(page: page, search: search, type: 'under_review');
    ...
    _busSub = _bus.stream.listen((event) {
      log(
        '📡 Bus:- MyReviewPostsBloc received for ${event.source} event.destination: ${event.destination}',
      );
      ...
      log('✅ Updating item: ${updatedItem.id} ${updatedItem.title}');
      log(
        '📡 Bus:- MyReviewPostsBloc received for ${event.source}: ${updatedItem.id} ${updatedItem.title}',
      );
