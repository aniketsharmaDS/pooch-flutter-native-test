...
// COMMUNITY
@RoutePage()
class MissingPoochListScreen extends StatefulWidget {
  const MissingPoochListScreen({super.key});

  @override
  State<MissingPoochListScreen> createState() => _MissingPoochListScreenState();
}

class _MissingPoochListScreenState extends State<MissingPoochListScreen> {
  late List<MissingPoochItemModel> _items;

  static const List<MissingPoochItemModel> _seedItems = <MissingPoochItemModel>[
    MissingPoochItemModel(
      ...
      badge: 'Missing',
      postImages: [
        'https://images.pexels.com/photos/5731866/pexels-phssoto-5731866.jpeg',
      ...
      description:
          'My pet Bella is missing since 2 days. She has a mark on her left leg.My pet Bella is missing since 2 days. She has a mark on her left leg.My pet Bella is missing since 2 days. She has a mark on her left leg.',
  ...
  void initState() {
    super.initState();
    _items = List<MissingPoochItemModel>.from(_seedItems);
  }

  // ignore: unused_element
  void _openDetail(int index) {
    final MissingPoochItemModel item = _items[index];

    Navigator.of(context).push(
      PageRouteBuilder<void>(
        transitionDuration: const Duration(milliseconds: 260),
        reverseTransitionDuration: const Duration(milliseconds: 220),
        pageBuilder: (_, _, _) => _MissingPoochDetailScreen(item: item),
        ...
        child: ListView.separated(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return Container(
              ...
              child: Center(child: AppText.h3('Missing Pooch Item ${item.id}')),
            );
...
class _MissingPoochDetailScreen extends StatelessWidget {
  final MissingPoochItemModel item;

  const _MissingPoochDetailScreen({required this.item});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      //   child: MissingPoochListItemCard(item: item, isDetailView: true),
      // ),
    );
  }
}

class MissingPoochItemModel {
  final String id;
  ...
  final bool showMissingBadge;
  final String disclaimer;
  final String? reward;

  const MissingPoochItemModel({
    required this.id,
    ...
    required this.showMissingBadge,
    required this.disclaimer,
    this.reward,
  });

  MissingPoochItemModel copyWith({
    String? id,
    String? userName,
    String? userImage,
    String? timeAgo,
    ...
    String? location,
    String? lastSeen,
    bool? showMissingBadge,
    String? disclaimer,
    String? reward,
  }) {
    return MissingPoochItemModel(
      id: id ?? this.id,
      ...
      showMissingBadge: showMissingBadge ?? this.showMissingBadge,
